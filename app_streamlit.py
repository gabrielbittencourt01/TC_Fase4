...
input_method = st.radio("", ["📂 Upload de arquivo (.xlsx)", "⌨️ Digitar manualmente os 60 valores"])

scaler = joblib.load("models/scaler.pkl")  # Usado para normalização se valores forem manuais
...
if input_method == "📂 Upload de arquivo (.xlsx)":
    ...
    if st.button("🔮 Prever com valores digitados"):
        try:
            values = [float(v.strip()) for v in manual_input.replace("\n", ",").split(",") if v.strip()]
